...
const interactionWithMulticall = async function () {    
    ...
    const multicallContract =  new Contract( process.env.SEPOLIA_MULTICALL_ADDRESS || "", 
    ...
    try {
        result = await multicallContract.callStatic.aggregate(inputs);
    } catch(e) {
        console.log(e)
    }

    console.log("blocknumber is", result.blockNumber.toString());
    ...
    for ( let index = 0; index < result.returnData.length; index++) {
        const balance = tokenInterface.decodeFunctionResult(fragment_balance, result.returnData[index]);
